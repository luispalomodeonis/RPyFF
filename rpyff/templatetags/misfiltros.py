...
def resalta(value, cadena):
   ...
   #print("Longitud:"+str(len(wordsList)))
   for index, word in enumerate(wordsList):
      ...
      if cadena.lower() in word.lower():
         ...
         reemplazar_por='<span style="color:blue">%s</span>' % result
         nueva_cadena=word.lower().replace(cadena.lower(),reemplazar_por)
         wordsList[index] = nueva_cadena
   result = " ".join([word for word in wordsList])
   return mark_safe(result)

def resalto(value, cadena):
   print("Value:'"+value+"' Cadena:"+cadena)
   wordsList = value.split()
   for index, word in enumerate(wordsList):
      print("Index:"+str(index)+" Word:"+word)
      if cadena.lower() in word.lower():
         ...
         if inicio == 0:
            ...
            result=word[inicio:fin].lower()
         #result=str(inicio)+result
         #result=word[inicio:fin]
         reemplazar_por='<span style="color:white">%s</span>' % result
         #reemplazar_por='<span style="color:white;background-color:orange">%s</span>' % result
         #reemplazar_por='<span style="color:orange">%s</span>' % result
         if inicio != 0:
            nueva_cadena=word.replace(cadena,reemplazar_por,1)
         else:
            nueva_cadena=word.lower().replace(cadena.lower(),reemplazar_por,1)
         #nueva_cadena=word.lower().replace(cadena.lower(),reemplazar_por)
         #nueva_cadena=word.replace(nueva_cadena,reemplazar_por,1)
         wordsList[index] = nueva_cadena
         word=wordsList[index][len(nueva_cadena):]
      print("Index:"+str(index)+" Word:"+word)
      if word.lower() == cadena.lower():
         #reemplazar_por='<span style="color:white">%s</span>' % cadena
         #reemplazar_por='<span style="color:white">%s</span>' % word
         #reemplazar_por='<span style="color:white;background-color:red">%s</span>' % word
         reemplazar_por='<span style="color:red">%s</span>' % word
         wordsList[index] = reemplazar_por
         word=wordsList[index][len(reemplazar_por):]
      print("Index:"+str(index)+" Word:"+word)
      if cadena in word:
         #reemplazar_por='<span style="color:white">%s</span>' % cadena
         #reemplazar_por='<span style="color:white;background-color:blue">%s</span>' % cadena
         reemplazar_por='<span style="color:blue">%s</span>' % cadena
         nueva_cadena=word.replace(cadena,reemplazar_por)
         wordsList[index] = nueva_cadena
         word=wordsList[index][len(nueva_cadena):]
      print("Index:"+str(index)+" Word:"+word)
...
def resaltayenlaza(value, args):
   ...
   for index, word in enumerate(wordsList):
      if word == cadena:
         ...
         nueva_cadena=word.lower().replace(cadena.lower(),reemplazar_por)
         #nueva_cadena=word.replace(cadena.lower(),reemplazar_por)
         wordsList[index] = nueva_cadena
   result = " ".join([word for word in wordsList])
   return mark_safe(result)
